# config.sh

# 模型名字
EXP_NAME="my_models1"

# ========================== 训练配置 ===========================
# 训练集路径
TRAIN_FILE="lwt"
# 训练参数
BATCH_SIZE=16
TOTAL_EPOCH=20
SAVE_EVERY=5
SR="48k"
VERSION="v2"
GPU=0

# 训练文件列表
TRAINING_FILES="logs/${EXP_NAME}/filelist.txt"  # 训练文件列表
SAVE_EVERY_EPOCH=10                      # 每多少个epoch保存一次模型
SAVE_EVERY_WEIGHTS="1"                   # 是否保存每个epoch的权重 (0不保存，1保存)

# 模型config.json输出
CONFIG_OUTPUT="logs/${EXP_NAME}/config.json"

# ============================= 推理配置 ========================
# 输入文件路径
INPUT_WAV="test-audio"
# 特征文件目录
FEATURE_DIR="./logs/$EXP_NAME/3_feature768"     
# 索引输出目录
INDEX_OUTPUT_DIR="./logs/$EXP_NAME"
# 特征维度              
FEATURE_DIM=768
 # 索引文件名前缀                                    
INDEX_PREFIX="added_index"                        

# 模型相关路径
INDEX_FILE="logs/$EXP_NAME/added_index.index"
# 自己选择模型
MODEL_WEIGHT="assets/weights/my_models1_e20_s860.pth"

# 分离人声伴奏输出路径
VOCAL_DIR="vocals"
ACCOM_DIR="accoms"
PREDICT_DIR="predicts"
MERGED_DIR="merged"

# 推理参数
F0_METHOD="rmvpe"           # f0提取方法: rmvpe
PITCH_SHIFT=0               # 音高偏移（半音），男生变女生用+，女生变男生用-
F0_GUIDE="True"             # 是否使用f0引导
SPEAKER_ID=0                # 说话人ID
CLUSTER_RATIO=0.7           # 聚类比例 ：想要更像目标调大，想要不刺耳、更稳定调小
DEVICE="cuda:0"             # 设备: cuda:0
AUTO_PREDICT=6              # 自动预测模式
NOISE_SCALE=0               # 噪声比例：0.1-0.3轻微加噪，更自然，也可以不加
PAD_INPUT=1                 # 是否填充输入
THREADS=0.8                 # 线程数

# 分离参数
DEMUCS_MODEL="mdx_extra"    # 分离模型: mdx_extra, mdx, vocal_分离等