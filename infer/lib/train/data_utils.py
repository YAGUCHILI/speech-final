...
class TextAudioCollateMultiNSFsid:
    ...
    def __call__(self, batch):
        ...
        # 获取 phone 的特征维度
        phone_sample = batch[0][2]
        if phone_sample.dim() == 2:
            phone_feat_dim = phone_sample.shape[1]
        else:
            phone_feat_dim = 1

        phone_padded = torch.FloatTensor(
            len(batch), max_phone_len, phone_feat_dim
        )
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            phone = row[2]
            if phone.dim() == 1:
                phone_padded[i, : phone.size(0), 0] = phone
            else:
...
class TextAudioLoader(torch.utils.data.Dataset):
    ...
    def get_audio_text_pair(self, audiopath_and_text):
        ...
        phone = phone[:min_len] if phone.dim() == 1 else phone[:min_len, :]
        pitch = pitch[:min_len]
